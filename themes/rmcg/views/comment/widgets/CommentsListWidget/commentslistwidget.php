	...
	<?php if (count($comments)): ?>
		...
		<?php foreach ($comments as $comment): ?>
			...
			<?php else: ?>
				...
				<?php $level = 1; ?>
			<?php endif; ?>
			<div class="well well-small comment-body"
				 id="comment_<?php echo $comment->id; ?>_<?php echo str_replace(' ', '_', $comment->creation_date); ?>"
				 style="margin-left: <?php echo(10 * $level); ?>px; " level="<?php echo $level; ?>">
				<a class="pull-left"
				   href="<?php echo $comment->getAuthorUrl() ?>"><?php echo $comment->getAuthorAvatar(); ?></a>
				<ul>
					<li><?php echo $comment->getAuthorLink(); ?></li>
					<li class="time icon-time">
						<time
							datetime="<?php echo str_replace(' ', '_', $comment->creation_date); ?>"><?php echo Yii::app()->getDateFormatter()->formatDateTime($comment->creation_date, "long", "short"); ?></time>
					</li>
